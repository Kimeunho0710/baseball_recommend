...
resource "aws_ssm_parameter" "jwt_secret" {
  name  = "/baseball/jwt/secret"
  type  = "SecureString"
  value = random_password.jwt.result
}

resource "aws_iam_role" "ecs_execution" {
  name = "baseball-ecs-execution-role"
...
resource "aws_s3_bucket" "frontend" {
  bucket        = "baseball-recommend-frontend-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
}
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ...
        StringLike = {
          "token.actions.githubusercontent.com:sub" = "repo:Kimeunho0710/baseball_recommend:*"
        }
      }
    }]
  })
