...
| `no match for platform in manifest: not found` | 그 도커 이미지에 **arm64 빌드가 없다.** 멀티아치 태그로 바꾸거나 `--platform linux/amd64` 로 빌드한다. `docker manifest inspect <이미지>` 로 지원 아키텍처를 확인할 수 있다 |
